...
(function (global) {
  ...
  function attach(target, options) {
    ...
    async function submitCooOrder(){
      ...
      var items=cart.map(function(c){return {name:c.name || c.key, key:c.key, qty:c.qty, price:c.price, recipeId:c.recipeId||''};});
    ...
    function addToCart(key) {
      ...
      if (existing) {
        ...
        cart.push({ key: key, qty: 1, price: inv.price, unit: inv.unit, recipeId: inv.recipeId || '' });
      }
      renderCart();
      renderPicker();
    }
        ...
        if (orderType === 'quick') {
          ...
          if (service && typeof service.recordSale === 'function') {
            ...
            const sale = {
              ...
              date: stamp, status: isRoomCharge ? 'pending' : 'completed', source: 'quick',
              roomNumber: isRoomCharge ? (room.room || null) : null,
              guestName: isRoomCharge ? (room.guest || null) : null,
              guestPhone: isRoomCharge ? (room.phone || null) : null,
            };
      ...
      try {
        if (service && typeof service.payOrder === 'function') {
          ...
          const sale = {
            ...
            notes: o.notes,
            date: stamp,
            status: isRoomCharge ? 'pending' : 'completed',
            source: 'tab',
