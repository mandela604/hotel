...
exports.pnl = asyncHandler(async (req, res) => {
  ...
  // ── Income: return detailed rows so frontend shows each booking/sale (no per-day aggregation) ──
  ...
  const income = incomeRows
    ...
    .map(r => ({
      id: r.id,
      date: r.date,
      department: DEPT_MAP[r.department] || r.department || 'Other',
      amount: r.amount,
      source: r.source || (r.department === 'Rooms & Bookings' ? 'booking' : r.department === 'Restaurant' ? 'Restaurant' : r.department === 'Pool Bar' ? 'Poolbar' : r.department === 'Gym' ? 'gym' : 'manual'),
      description: r.description || '',
      method: r.method || '',
      recordedBy: r.recordedBy || '',
      autoGenerated: r.autoGenerated !== undefined ? r.autoGenerated : true,
    }))
