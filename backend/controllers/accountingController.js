...
exports.pnl = asyncHandler(async (req, res) => {
  ...
  for (const r of incomeRows) {
    ...
    const key = d + '|' + dept;
    if (!grouped[key]) {
      grouped[key] = { id: key, date: d, department: dept, amount: 0, source: r.source || (dept === 'Front Desk' ? 'booking' : dept === 'Restaurant' ? 'Restaurant' : dept === 'Pool Bar' ? 'Poolbar' : dept === 'Gym' ? 'gym' : 'manual'), description: '', method: '', recordedBy: '', autoGenerated: r.autoGenerated !== undefined ? r.autoGenerated : true, incomeItems: [] };
    }
    grouped[key].amount += Number(r.amount) || 0;
    grouped[key].incomeItems.push({ id: r.id || key+'-'+grouped[key].incomeItems.length, date: r.date, department: dept, description: r.description || '', amount: Number(r.amount)||0, method: r.method||'', recordedBy: r.recordedBy||'', source: r.source||'' });
  }
