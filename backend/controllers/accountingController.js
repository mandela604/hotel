...
async function aggregateRoomRevenue(from, to) {
  const match = { guest: { $ne: '' }, status: { $in: ['reserved', 'checkedin', 'checkout'] } };
  if (from || to) {
    match.checkin = {};
  ...
  return bookings.map(b => {
    ...
    const lastPayment = (b.payments && b.payments.length) ? b.payments[b.payments.length - 1] : null;
...
exports.pnl = asyncHandler(async (req, res) => {
  ...
  const [roomIncome, restIncome, poolIncome, gymIncome, manualIncome, expenses, prs] = await Promise.all([
    ...
    IncomeEntry.find({ autoGenerated: { $ne: true } }).sort({ date: -1 }).catch(err => { console.error('[Accounting] Manual income query failed:', err.message); return []; }),
    ExpenseEntry.find().sort({ date: -1 }).catch(err => { console.error('[Accounting] Expense query failed:', err.message); return []; }),
    PurchaseRequest.find({ approvalStage: { $in: ['fulfilled', 'sent_to_store'] } }).sort({ createdAt: -1 }).catch(err => { console.error('[Accounting] PurchaseRequest query failed:', err.message); return []; }),
  ]);

  // ── Income: aggregate by shift/calendar day + department ──
  ...
  for (const row of incomeRows) {
    ...
    const key = shiftKey(row.date) + '|' + (DEPT_MAP[row.department] || row.department || 'Other');
    if (!incMap[key]) incMap[key] = { date: shiftKey(row.date), department: DEPT_MAP[row.department] || row.department || 'Other', amount: 0 };
    incMap[key].amount += row.amount;
  }
