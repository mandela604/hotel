...
exports.pnl = asyncHandler(async (req, res) => {
  ...
  const incomeRows = [...roomIncome, ...restIncome, ...poolIncome, ...gymIncome, ...manualIncome.map(e => ({
    id: e._id ? e._id.toString() : e.id, date: shiftKey(fmtDate(e.date)), department: e.department || 'Other', amount: e.amount || 0, autoGenerated: false, source: e.source || 'manual', description: e.description || '', method: e.method || '', recordedBy: e.recordedBy || '',
  }))];

  // Normalize dept + group date through shiftKey, then sum
  const grouped = {};
  for (const r of incomeRows) {
    ...
    const key = d + '|' + dept;
    if (!grouped[key]) {
      grouped[key] = { id: key, date: d, department: dept, amount: 0, source: r.source, description: '', method: '', recordedBy: '', autoGenerated: r.autoGenerated !== undefined ? r.autoGenerated : true };
    }
